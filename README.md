...
Cross-session work memory shared by Claude Code, Codex, OpenCode and Pi. It reads each agent's local session store (read-only), normalizes it into a single worklog, injects a project brief at session start, and exposes hybrid (lexical + semantic) recall through MCP.
...
3. **Context on first call.** When you open a session, Nimrod injects a project *brief* — recent work and its outcomes — into the model context, formatted for the respective agent. The next agent picks up where the previous one left off.
...
```
 Claude Code / Codex / OpenCode / Pi
  ...
  ingest ─────────────────────► ingest reads local agent stores (read-only)
        ...
        ▼
 brief injected on the first model call   ◄────────  MCP: work_search /
        │                                          work_context / ...
        ▼
 agent continues with the previous session's context
```
...
| Agent      | Context injection                              | Capture                                              | MCP registration                     |
|------------|------------------------------------------------|------------------------------------------------------|--------------------------------------|
...
The `project` argument accepts a path, a project folder name, or `*` for every project.
...
| `nimrod context [--project] [--limit]` | The handoff brief for a project (what context injection delivers). |
